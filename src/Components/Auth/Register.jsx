...
import {
  User, Phone, Mail, ShieldCheck, ArrowRight
} from "lucide-react";
...
const RegisterForm = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-xl p-6 sm:p-8 space-y-5">

        <h2 className="text-2xl sm:text-3xl font-black">Create Account</h2>

        {[
          { id: "name", icon: User, placeholder: "Full Name" },
          { id: "mobile", icon: Phone, placeholder: "Mobile Number" },
          { id: "email", icon: Mail, placeholder: "Email Address" },
        ].map((f) => (
          <div key={f.id} className="relative">
            <f.icon className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300" />
            <input
              placeholder={f.placeholder}
              onChange={(e) =>
                setData({ ...data, [f.id]: e.target.value })
              }
        ...
        <motion.div
          ...
          `}
        >
          <span className="font-semibold">I am a farmer</span>
        ...
        <button
          ...
          className={`w-full py-4 rounded-2xl font-bold flex justify-center gap-2
            ${valid && robot ? "bg-green-600 text-white" : "bg-gray-200 text-gray-400"}
          `}
        >
          Register <ArrowRight />
        </button>
